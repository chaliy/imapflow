 ...
 * @typedef {Object} ImapFlowOptions
 * @property {String} host Hostname of the IMAP server
 * @property {Number} port Port number for the IMAP server
 * @property {Boolean} [secure=false] Should the connection be established over TLS.
 *      If `false` then connection is upgraded to TLS using STARTTLS extension before authentication
 * @property {String} [servername] Servername for SNI (or when host is set to an IP address)
 * @property {Boolean} [disableCompression=false] if `true` then client does not try to use COMPRESS=DEFLATE extension
 * @property {Object} auth Authentication options. Authentication is requested automatically during <code>connect()</code>
 * @property {String} auth.user Usename
 * @property {String} [auth.pass] Password, if using regular authentication
 * @property {String} [auth.accessToken] OAuth2 Access Token, if using OAuth2 authentication
 * @property {IdInfoObject} [clientInfo] Client identification info
 * @property {Boolean} [disableAutoIdle=false] if `true` then IDLE is not started automatically. Useful if you only need to perform specific tasks over the connection
 * @property {Object} [tls] Additional TLS options (see [Node.js TLS connect](https://nodejs.org/api/tls.html#tls_tls_connect_options_callback) for all available options)
 * @property {Boolean} [tls.rejectUnauthorized=true] if `false` then client accepts self-signed and expired certificates from the server
 * @property {String} [tls.minVersion=TLSv1.2] latest Node.js defaults to *'TLSv1.2'*, for older mail servers you might need to use something else, eg *'TLSv1'*
 * @property {Number} [tls.minDHSize=1024] Minimum size of the DH parameter in bits to accept a TLS connection
 * @property {Object} [logger] Custom logger instance with `debug(obj)`, `info(obj)`, `warn(obj)` and `error(obj)` methods. If not provided then ImapFlow logs to console using pino format
 * @property {Boolean} [logRaw=false] If true then log data read from and written to socket encoded in base64
 * @property {Boolean} [emitLogs=false] If `true` then in addition of sending data to logger, ImapFlow emits 'log' events with the same data
 * @property {Boolean} [verifyOnly=false] If `true` then logs out automatically after successful authentication
 * @property {Boolean} [proxy] Optional proxy URL. Supports HTTP CONNECT (`http://`, `https://`) and SOCKS (`socks://`, `socks4://`, `socks5://`) proxies
 * @property {Boolean} [qresync=false] If true, then enables QRESYNC support. EXPUNGE notifications will include `uid` property instead of `seq`
 * @property {Number} [maxIdleTime] If set, then breaks and restarts IDLE every maxIdleTime ms
 * @property {Boolean} [disableBinary=false] If true, then ignores the BINARY extension when making FETCH and APPEND calls
 */
...
class ImapFlow extends EventEmitter {
    ...
    /**
     * @param {ImapFlowOptions} options IMAP connection options
     */
    constructor(options) {
    ...
    async reader() {
        ...
        while ((data = this.streamer.read()) !== null) {
            ...
            if (parsed.tag === '+' && this.commandParts.length) {
                let content = Buffer.from(this.commandParts.shift(), 'binary');
                this.write(content);
     ...
     * @returns {AsyncGenerator<FetchMessageObject, never, void>} Array of message data objects
     *
     * @example
     * let mailbox = await client.mailboxOpen('INBOX');
